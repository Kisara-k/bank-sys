...
BEGIN
    IF NEW.status = 'expired' THEN
        INSERT INTO account_status_log (account_id, set_status_action, date)
        VALUES (NEW.account_id, 'blocked', NOW());
    END IF;
END;
...
BEGIN
    ...
    SELECT SUM(amount) INTO total_amount_paid 
    ...
    IF total_amount_paid >= (SELECT amount FROM loans WHERE loan_id = NEW.loan_id) THEN
        UPDATE loans SET status = 'paid' WHERE loan_id = NEW.loan_id;
    END IF;
END;
...
-- update saving and checking account when update account (monthly withdraw decremented in procedure)
DELIMITER //
CREATE TRIGGER update_account_after_saving_or_checking_update
AFTER UPDATE ON account
FOR EACH ROW
BEGIN
    IF EXISTS (SELECT 1 FROM saving_account WHERE account_id = NEW.account_id) THEN
        UPDATE saving_account SET balance = NEW.balance WHERE account_id = NEW.account_id;
    ELSEIF EXISTS (SELECT 1 FROM checking_account WHERE account_id = NEW.account_id) THEN
